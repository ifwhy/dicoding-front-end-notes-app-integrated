...
document.addEventListener("DOMContentLoaded", () => {
  ...
  judulCatatan.addEventListener("input", (event) =>
    handleFocus(event, "Judul Wajib Diisi!"),
  );
  isiCatatan.addEventListener("input", (event) =>
    handleFocus(event, "Isi catatan tidak boleh kosong!"),
  );
  ...
  const validasiJudul = (event) =>
    handleValidation(
      event,
      6,
      50,
      "Judul wajib diisi!",
      "Judul minimal 6 karakter!",
      "Judul maksimal 50 karakter!",
    );
  const validasiIsi = (event) =>
    handleValidation(
      event,
      10,
      null,
  ...
  judulCatatan.addEventListener("input", validasiJudul);
  judulCatatan.addEventListener("invalid", validasiJudul);
  isiCatatan.addEventListener("input", validasiIsi);
  ...
  judulCatatan.addEventListener("input", (event) =>
    handleBlur(event, "Judul Valid!"),
  );
  isiCatatan.addEventListener("input", (event) =>
    handleBlur(event, "Isi catatan valid!"),
  );
  ...
  isiCatatan.addEventListener("input", () => {
    ...
    charCount2.textContent =
      remainingChars <= 0 ? `` : `Kurang ${remainingChars} karakter lagi`;
  });
